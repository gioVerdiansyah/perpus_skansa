<x-app-layout>
    ...
    <div class="container mx-auto mt-10">
        <div class="max-w-md mx-auto bg-gray-900 p-5 rounded-md shadow-md">
            ...
            <form action="{{ route('publisher.update', $publisher) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="form-textarea mt-1 block w-full rounded-md dark:bg-gray-700 dark:border-gray-600 @error('address') border-red-500 @enderror">{{ old('address', $publisher->address) }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <img src="{{ asset('storage/image/logo-publisher/' . $publisher->logo) }}" alt="logo publisher"
                        id="img" width="70">
                    ...
                    <input type="file" name="logo" id="logo"
                        ...
                        accept="image/*" value="{{ $publisher->logo }}"
                        onchange="
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="since" id="since"
                        ...
                        value="{{ old('since', $publisher->since) }}">
                    @error('since')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="5"
                        class="form-textarea mt-1 block w-full rounded-md dark:bg-gray-700 dark:border-gray-600 @error('description') border-red-500 @enderror">{{ old('description', $publisher->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
